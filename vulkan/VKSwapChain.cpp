...
VKSwapChain::VKSwapChain(VKDevice * vkDevice, VKSurface * vkSurface) :
	device(vkDevice->device)
{
	CheckSurfaceStats(vkDevice->physicalDevice, vkSurface->surface);
	...
	global::windowWidth = extent.width;
	global::windowHeight = extent.height;

	VkSwapchainCreateInfoKHR swapchainCI = {};
	swapchainCI.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR;
	swapchainCI.pNext = nullptr;
	swapchainCI.flags = 0;
	swapchainCI.surface = vkSurface->surface;
	...
	VK_CHECK_RESULT(vkCreateSwapchainKHR(vkDevice->device, &swapchainCI, nullptr, &swapChain));
	...
	if (oldSwapChain != VK_NULL_HANDLE)
	{
		for (size_t i = 0; i < swapChainImageViews.size(); ++i) {
			if (swapChainImageViews[i] != VK_NULL_HANDLE) {
				vkDestroyImageView(vkDevice->device, swapChainImageViews[i], nullptr);
				swapChainImageViews[i] = VK_NULL_HANDLE;
			}
		}
		vkDestroySwapchainKHR(vkDevice->device, oldSwapChain, nullptr);
	}

	uint32_t imageCount = 0;
	VK_CHECK_RESULT(vkGetSwapchainImagesKHR(vkDevice->device, swapChain, &imageCount, nullptr));
	swapChainImages.resize(imageCount);
	VK_CHECK_RESULT(vkGetSwapchainImagesKHR(vkDevice->device, swapChain, &imageCount, swapChainImages.data()));
	...
	for (uint32_t i = 0; i < imageCount; i++)
	{
		...
		VK_CHECK_RESULT(vkCreateImageView(vkDevice->device, &colorAttachmentView, nullptr, &swapChainImageViews[i]));
	}
}
